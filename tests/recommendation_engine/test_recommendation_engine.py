...
from src.recommendation_engine.rule_based_risk import (
    compute_rule_based_risk,
    _trend_risk_component,
)
...
def test_trend_component_distinguishes_volatile_from_smooth_uptrend():
    ...
    # volatile series has a near-flat/unpredictable slope,
    # not a clean uptrend
...
def test_compute_rule_based_risk_flags_low_doi_high_volatility_as_high():
    doi_table = pd.DataFrame({
        "product_id": ["A"],
        "days_of_inventory": [3.0],
        "latest_stock": [10],
    })
    ...
    forecast_results = {
        "A": {
            "forecast": pd.Series([10, 11, 12, 13, 14])
        }
    }
...
def test_recommend_reduce_inventory_skips_erratic_demand():
    doi_row = pd.Series({
        "product_id": "A",
        "days_of_inventory": 150,
    })

    assert recommend_reduce_inventory(
        doi_row,
        "Z",
        "Widget",
    ) is None  # erratic -- might be legit buffer
...
def test_recommend_increase_safety_stock_only_for_erratic_and_risky():
    ...
    assert recommend_increase_safety_stock(
        risk_row,
        "X",
        "Widget",
    ) is None  # stable demand

    assert recommend_increase_safety_stock(
        risk_row,
        "Z",
        "Widget",
    ) is not None  # erratic + risky
...
def test_recommend_price_change_skips_suspect_extreme():
    row = pd.Series({
        "product_id": "A",
        "classification": "suspect_extreme",
...
def test_run_recommendations_end_to_end():
    ...
    if recommendations:
        ...
        assert priorities == sorted(
            priorities,
            key=lambda p: priority_values[p],
        )