...
CANONICAL_FIELDS: list[CanonicalField] = [
    ...
    CanonicalField(
        ...
        requirement=FieldRequirement.OPTIONAL,
        description=(
            "Cost price per unit. Never defaulted -- a fabricated "
    ...
    CanonicalField(
        ...
        requirement=FieldRequirement.OPTIONAL,
        description=(
            "Days between placing a reorder and receiving stock. "
...
OPTIONAL_FIELDS: list[str] = [
    f.name for f in CANONICAL_FIELDS
    if f.requirement == FieldRequirement.OPTIONAL
]

ALL_FIELDS: list[str] = REQUIRED_FIELDS + OPTIONAL_FIELDS

_FIELD_LOOKUP: dict[str, CanonicalField] = {
    f.name: f for f in CANONICAL_FIELDS
}
...
def get_field(name: str) -> CanonicalField:
    if name not in _FIELD_LOOKUP:
        raise KeyError(
            f"'{name}' is not a canonical field. Valid fields: {ALL_FIELDS}"
        )
...
class CanonicalRecord(BaseModel):
    ...
    @field_validator("unit_price", "unit_cost", "current_stock", "lead_time_days")
    @classmethod
    def numeric_optional_not_negative(
...
def field_is_available(df, field_name: str) -> bool:
    """
    Whether a canonical field genuinely has usable data -- not just
    whether the column exists.

    apply_mapping() always creates every canonical column (filled with
    None when unmapped), so checking column presence alone would always
    return True post-mapping.

    This checks for:
    1. The canonical column actually existing.
    2. At least one real, non-null value being present.
    """
    if field_name not in df.columns:
        return False

    return df[field_name].notna().any()